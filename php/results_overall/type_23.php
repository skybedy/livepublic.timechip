<?php
	...
	while($data1 = $sth1->fetchObject()){//cyklus, v kterém se vyberou všecky časy konkrétního závodníka
		...
		$rank_background = "";
		if($data1->rank_category <= 3){
		    $rank_background = ' class="rank_background"';
		}
		
		if($data1->rank_gender <= 3){
		    $rank_background = ' class="rank_background"';
		}
		
		$str .= '<tr'.$rank_background.'><td class="text-center"><b>'.$poradi.'</b></td><td class="text-center">'.$data1->ids.'</td><td><a onclick="detail_cipu_lahofer('.$data1->cip.','.$this->race_id.','.$this->race_year.')" href="'.$hash_url.'vysledky">'.$dbdata3->jmeno.'</a></td><td class="text-center">'.$dbdata3->rocnik.'</td><td>'.$this->NahrazkaPomlcky($dbdata3->nazev_tymu).'</td><td class="text-center">'.$dbdata3->stat.'</td><td class="text-center">'.$dbdata3->nazev_kategorie.'</td><td class="text-center"><b>'.$data1->rank_category.'</b></td><td class="text-center">'.$dbdata3->pohlavi.'</td><td class="text-center"><b>'.$data1->rank_gender.'</b></td>';
