<?php
		...
		if($sth1->rowCount()){
		    ...
		    $str .= '<thead><tr class="header"><th class="text-center">Poř</th><th class="text-center">St.č</th><th class="text-left">Jméno</th><th class="text-center">Ročník</th><th class="text-center">Stát</th>';
                    $str .= '<th class="text-center">Ran</th>';
			...
			$sql3 = "SELECT CONCAT_WS(' ',$this->sqlosoby.prijmeni,$this->sqlosoby.jmeno) AS jmeno,$this->sqlosoby.rocnik,$this->sqlosoby.psc AS stat,$this->sqlosoby.pohlavi,$this->sqlkategorie.kod_k AS nazev_kategorie,tymy.nazev_tymu FROM $this->sqlzavod,$this->sqlosoby,$this->sqlkategorie,tymy WHERE $this->sqlzavod.ids = $data1->ids AND $this->sqlzavod.ido = $this->sqlosoby.ido AND $this->sqlzavod.id_tymu = tymy.id_tymu AND $this->sqlzavod.id_kategorie = $this->sqlkategorie.id_kategorie";
			//echo $sql3."\n";
			...
			if($sth3->rowCount()){
			    ...
			    $str .= '<tr><td class="text-center"><b>'.$poradi.'</b></td><td class="text-center">'.$data1->ids_alias.'</td><td>'.$dbdata3->jmeno.'</td><td class="text-center">'.$dbdata3->rocnik.'</td><td class="text-center">'.$dbdata3->stat.'</td>';
                            $str .='<td class="text-center">'.$data1->ran.'</td>';
