<?php
    ...
    if($sth1->rowCount()){
	//$str .= '<br><br><h4 class="headline-results">'.$this->race_name.', výsledky bez rozdílu kategorií</h4>';
        $str .= '<br><br><h4 class="headline-results text-left float_left">'.$this->race_name.' - Výsledky '.$this->event_name_bez_carky.', '.$this->misto_zavodu.' '.$this->datum_zavodu.'</h4>';
	$str .= '<table  id="table2excel" class="table table-bordered table-hover table-striped table_vysledky">';
	$str .= '<thead><tr class="header"><th class="text-center">Poř</th><th class="text-center">St.č</th><th>Jméno</th><th class="text-center">Ročník</th><th>Tým/Bydliště</th><th class="text-center">Stát</th><th class="text-center">Kola</th><th class="text-center">Čas</th><th class="text-center">Odstup</th>';
	$str .= '</tr></thead><tbody>';
	$poradi = 1;
