<?php  
	...
	$str .= '<table class="table table-bordered table-hove table_vysledky"  id="table2excel">';
	$str .= '<thead><tr class="header"><th class="text-center">#</th><th>Tým</th><th>Kategorie</th><th>Člen týmu</th><th class="text-center">Ročník</th><th class="text-center">St.č</th><th class="text-center">Kola</th><th class="text-center">Km</th><th class="text-center">Celkový čas</th><th class="text-center">Odstup</th>';
	$str .= '</tr></thead><tbody>';
	$poradi = 1;
	...
	while($data1 = $sth1->fetchObject()){ //začneme vypisovat první dotaz
		...
		($dbdata2->distance_overall != '00:00:00.00') ? ($distance_overall = $dbdata2->distance_overall) : ($distance_overall = '-'); //pokud nemá odstup, tak je první a bude tam pomlčka
	        ($dbdata2->distance_overall != $data1->finish_time) ? ($distance_overall = $dbdata2->distance_overall) : ($distance_overall = '-'); 
	    ...
	    $sql3 = "SELECT CONCAT_WS(' ',osoby.prijmeni,osoby.jmeno) AS jmeno,osoby.rocnik,$this->sqlzavod.cip,$this->sqlzavod.ids_alias FROM osoby,$this->sqlzavod WHERE $this->sqlzavod.ids = :ids AND $this->sqlzavod.ido = osoby.ido ORDER BY $this->sqlzavod.id";
		...
		while($dbdata3 = $sth3->fetchObject()){
			...
			$str .= '<td rowspan="'.$pocet_clenu.'"><a onclick="detail_ids_fichtly24('.$data1->ids_alias.','.$this->race_id.','.$this->race_year.')" href="'.$hash_url.'vysledky">'.$data1->nazev_tymu.'</a></td>';
			$str .= '<td rowspan="'.$pocet_clenu.'">'.$data1->nazev_kategorie.'</td>';
			$str .= '<td><a onclick="detail_cipu_fichtly24('.$dbdata3->cip.','.$this->race_id.','.$this->race_year.')" href="'.$hash_url.'vysledky">'.$dbdata3->jmeno.'</a></td>';
			$str .= '<td class="text-center">'.$dbdata3->rocnik.'</td>';
			//$cip = $dbdata3->cip;
			$cip = $dbdata3->ids_alias;
			$str .= '<td class="text-center">'.$cip.'</td>';
