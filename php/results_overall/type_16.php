<?php  
    /* 
     * CROSS Country hobby týmy.. nejsou motorky ani body
     */
	...
	$str .= '<thead><tr class="header"><th class="text-center">#</th><th class="text-center">St.č</th><th>Tým</th><th class="text-center">Ročník</th><th class="text-center">Čip</th><th>Kategorie</th><th class="text-center">Čas</th><th class="text-center">Kolo</th><th class="text-center">Odstup</th><th class="text-center">Km/h</th><th class="text-center">Nejlepší čas</th>';
	$str .= '</tr></thead><tbody>';
	$poradi = 1;
	...
	//if($poradi == 1) $max_time_order = $dbdata1->time_order;// nejvyšší počet časů pro počítání odstupů
	    ...
	    $sql2 = "SELECT CONCAT_WS(' ',osoby.prijmeni,osoby.jmeno) AS jmeno,osoby.psc AS stat,osoby.rocnik,$this->sqlzavod.cip "
		...
		while($dbdata2 = $sth2->fetchObject()){
			...
			$str .= '<td rowspan="'.$pocet_clenu.'" class="text-center">'.$dbdata1->ids.'</td>';
			$str .= '<td>'.$dbdata2->jmeno.'</td>';
			$str .= '<td class="text-center">'.$dbdata2->rocnik.'</td>';
			$str .= '<td class="text-center">'.$dbdata2->cip.'</td>';
			$str .= '<td rowspan="'.$pocet_clenu.'">'.$dbdata1->nazev_kategorie.'</td>';
			$str .= '<td rowspan="'.$pocet_clenu.'" class="text-center">'.$dbdata1->finish_time.'</td>';
			$str .= '<td rowspan="'.$pocet_clenu.'" class="text-center">'.$dbdata1->pocet_kol.'</td>';
			$str .= '<td rowspan="'.$pocet_clenu.'" class="text-center">'.$distance_overall.'</td>';
			$str .= '<td rowspan="'.$pocet_clenu.'" class="text-center">'.round($celkova_vzdalenost / $dbdata1->total_lap_time_sec * 3600,1).'</td>';
			$str .= '<td rowspan="'.$pocet_clenu.'" class="text-center">'.$dbdata1->best_lap_time.'</td>';
    ...
    /*
     * tohle v týmech zatím udělat neumím
