<?php
      ...
      /* Adrenalic Cup */
    if($this->laps_only){
	require_once 'type_30_laps_only.php';
    }
    else{
	$sql1 = "SELECT "
                    . "tymy.nazev_tymu,"
                ...
                . "WHERE "
                    ...
                    . "$this->sqlvysledky.reader LIKE 'CIL' AND "    
                    . "$this->sqlvysledky.lap_count <= $this->time_order " //tohle by mělo zajistit, že se vybere SUM pouze z počtu kol daného požadovaným time_order
	...
	if($sth1->rowCount()){
	    ...
	    $str .= '<table class="table table-striped table-hover table-bordered orlice_cup">';
            ...
            $str .= '<thead><tr class="header"><th rowspan="2" class="text-center">#</th><th rowspan="2" class="text-center">St.č</th><th class="text-left" rowspan="2">Tým</th><th rowspan="2" class="text-center">Pen</th><th rowspan="2" class="text-center">Celkový<br>čas</th><th rowspan="2" class="text-center">Odstup</th>';
		...
		//$str .= '<th class="text-left"><div style="float:left" class="jmeno_ac">Jméno</div><div class="cas_ac">Čas</div></th>';
		$xx++;
	    }
	    $str .= '</tr>'; 
	    $str .= '</thead><tbody>';
	    ...
	    while($dbdata1 = $sth1->fetchObject()){//cyklus, v kterém se vyberou všecky časy konkrétního týmu
                if($dbdata1->pocet_kol == $this->time_order){
                    ...
                    //$str .= '<td class="text-center"><b>'.$dbdata1->nazev_kategorie.'</b></td>';
                    //$str .= '<td class="text-center">'.$dbdata1->rank_category.'</td>';
                    if($dbdata1->pocet_kol == 5){
                     ...
                     $str .= '<td class="text-center"><b>'.substr($dbdata1->finish_time,1,-3).'</b></td>';
                    $str .= ($distance_time != '00:00:00.00') ?  ('<td class="text-center"><b>'.substr($distance_time,1,-3).'</b></td>') : ('<td class="text-center">...</td>');
                    $sql2 = "SELECT * FROM $this->sqlvysledky WHERE ids = $dbdata1->ids AND false_time IS NULL AND lap_only IS NULL AND reader = 'CIL' ORDER BY race_time ASC LIMIT 0,$this->time_order";
                    ...
                    while($val2 = $sth2->fetchObject()){
                        if($this->time_order == 1){//pokud je to první čas 
                            ...
                            //$str .= '<td><div class="jmeno_ac">'.$zavodnik_pole[$val2->cip]->jmeno.'</div> <div class="cas_ac">'.$val2->lap_time.'</div></td>';
                        }
