<?php 
	...
	public function __construct() {
	    ...
	    $sql2 = "SELECT "
		      ...
		      . "$this->sqlzavod.id_etapy = $this->sqlvysledky.id_etapy AND "
	    ...
	    $sth2->execute(Array(':cip' => $_GET['cip'],':id_etapy' => $_GET['id_etapy']));
...
<head>
    <title><?php echo $_GET['cip'].'@'.$dbdata2->jmeno ?></title>
    <meta charset="UTF-8" />
...
<?php	    
		...
		$str .= 'Celkový čas: <span class="color_red">'.$dbdata2->finish_time.'</span><br />';
		$str .= 'Průměrná rychlost: <span class="color_red">'.round($celkova_vzdalenost / $dbdata2->total_lap_time_sec * 3600,1).' km/h</span><br />';
		$str .= '</p><br />';
	    }
	    $sql1 = "SELECT $this->sqlvysledky.* FROM $this->sqlvysledky,$this->sqlzavod WHERE $this->sqlvysledky.false_time IS NULL AND $this->sqlvysledky.cip = :cip AND $this->sqlvysledky.cip = $this->sqlzavod.cip AND $this->sqlzavod.id_etapy = :id_etapy AND $this->sqlzavod.id_etapy = $this->sqlvysledky.id_etapy ORDER BY race_time";
	   // echo $sql1;
	    $sth1 = $this->db->prepare($sql1);
	    $sth1->execute(Array(':cip' => $_GET['cip'],':id_etapy' => $_GET['id_etapy']));
	    if($sth1->rowCount()){
		...
		while($dbdata1 = $sth1->fetchObject()){
		    $prumerna_rychlost = '-';
		    if($dbdata1->lap_time_sec > 0 AND $kolo > 1){
			$prumerna_rychlost = round($this->delka_kola/$dbdata1->lap_time_sec*3600,1);
		    }
			$str .= '<tr>';
			    $str .= '<td>'.($dbdata1->time_order-1).'</td>';
			    $str .= '<td>'.substr($dbdata1->lap_time,1).'</td>';
			    $str .= '<td>'.substr($dbdata1->race_time,1).'</td>';
