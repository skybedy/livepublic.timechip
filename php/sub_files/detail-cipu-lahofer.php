...
<?php 
	...
	public function __construct() {
		...
		$str .= '<p>Počet časů: <span class="color_red">'.$dbdata2->lap_count.'</span><br />';
		$str .= 'Celkový čas: <span class="color_red">'.$dbdata2->total_lap_time.'</span><br />';
		$str .= '</p><br />';
	    }
	    $sql1 = "SELECT $this->sqlvysledky.* FROM $this->sqlvysledky,$this->sqlzavod WHERE $this->sqlvysledky.false_time IS NULL AND $this->sqlvysledky.lap_only IS NULL AND $this->sqlvysledky.cip = :cip AND $this->sqlvysledky.cip = $this->sqlzavod.cip ORDER BY race_time_sec ASC";
	    $sth1 = $this->db->prepare($sql1);
	    $sth1->execute(Array(':cip' => $_GET['cip']));
	    if($sth1->rowCount()){
		$row_count = $sth1->rowCount();
		$str .= '<table class="table table-bordered table-hover table-striped text-center detail_ids">';
		$str .= '<thead><tr class="header"><th class="text-center">Čas</th><th class="text-center">Čas úseku</th><th class="text-center">#C</th><th class="text-center">#K</th><th class="text-center">Čas závodu</th><th class="text-center">#C</th><th class="text-center">#K</th><th class="text-center">Denní čas</th></tr></thead><tbody>';
		$kolo = 1;
		while($dbdata1 = $sth1->fetchObject()){
			$str .= '<tr>';
			    $str .= '<td>'.($dbdata1->time_order).'</td>';
			    $str .= '<td>'.$dbdata1->lap_time.'</td>';
			    $str .= '<td>'.$dbdata1->rank_overall_lap.'</td>';
			    $str .= '<td>'.$dbdata1->rank_category_lap.'</td>';
			    $str .= '<td>'.$dbdata1->race_time.'</td>';
			    $str .= '<td>'.$dbdata1->rank_overall.'</td>';
			    $str .= '<td>'.$dbdata1->rank_category.'</td>';
			    $str .= '<td>'.$dbdata1->day_time.'</td>';
			$str .= '</tr>';
		    $kolo++;
		}
		$str .= '</tbody></table>';
		$str .= "<p>#C - celkové pořadí času<br>"; 
		$str .= "#K - pořadí času v kategorii</p>"; 
	    }
