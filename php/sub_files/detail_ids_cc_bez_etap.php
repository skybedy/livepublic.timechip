...
<?php 
    ...
    $str .= '<h3>Startovní číslo '.$dbdata2->ids_alias.'</h3>';
    ...
    $sql1 = "SELECT $this->sqlvysledky.*,CONCAT_WS(' ',osoby.prijmeni,osoby.jmeno) AS jmeno FROM $this->sqlvysledky,$this->sqlzavod,osoby WHERE "
			  . "$this->sqlvysledky.false_time IS NULL AND $this->sqlvysledky.time_order > 1 AND $this->sqlvysledky.ids = :ids AND $this->sqlvysledky.cip = $this->sqlzavod.cip AND $this->sqlzavod.ido = osoby.ido "
			  . "ORDER BY race_time";
			  //echo $sql1;
	...
	$str .= '<thead><tr class="header"><th class="text-center">Kolo</th><th class="text-center">St.č</th><th>Jméno</th><th class="text-center">Čas kola</th><th class="text-center">Čas závodu</th><th class="text-center">Denní čas</th><th class="text-center">Km/h</th></tr></thead><tbody>';
       $poradi = 1;
	while($dbdata1 = $sth1->fetchObject()){
	    ...
	    $str .= '<td style="padding-left:0" class="text-left"><a href="#" style="text-decoration:none" onclick="detail_cipu_cc_bez_etap('.$dbdata1->cip.','.$this->race_id.','.$this->race_year.')">'.$dbdata1->jmeno.'</a></td>';
...
function detail_cipu_cc_bez_etap(cip,race_id,race_year) {
    window.open("detail_cipu_cc_bez_etap.php?cip="+cip+"&race_id="+race_id+"&race_year="+race_year);
}
