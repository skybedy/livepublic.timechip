<?php	
	...
	public function __construct() {
	    ...
	    $sth2->execute(Array(':id_etapy' => $_GET['id_etapy'], ':ids' => $_GET['ids']));
	    $dbdata2 = $sth2->fetchObject();
?>
<!DOCTYPE html>
<html lang="cs" dir="ltr">
<head>
    <title>Startovní číslo <?php echo $_GET['ids'] ?></title>
    <meta charset="UTF-8" />
...
<?php 
    ...
    $str .= '<h3>Startovní číslo '.$_GET['ids'].'</h3>';
    ...
    $sql1 = "SELECT $this->sqlvysledky.*,CONCAT_WS(' ',osoby.prijmeni,osoby.jmeno) AS jmeno FROM $this->sqlvysledky,$this->sqlzavod,osoby WHERE "
			  . "$this->sqlvysledky.time_order > 1 AND $this->sqlvysledky.false_time IS NULL AND $this->sqlvysledky.ids = :ids AND $this->sqlvysledky.cip = $this->sqlzavod.cip AND $this->sqlzavod.ido = osoby.ido AND $this->sqlvysledky.id_etapy = :id_etapy "
			  . "ORDER BY race_time";
    $sth1 = $this->db->prepare($sql1);
    $sth1->execute(Array(':ids' => $_GET['ids'],':id_etapy' => $_GET['id_etapy']));
    $posledni_kolo = false;
    if($sth1->rowCount()){
	...
	$str .= '<thead><tr class="header"><th class="text-center">Kolo</th><th class="text-center">St.č</th><th>Jméno</th><th class="text-center">Čas kola</th><th class="text-center">Čas závodu</th><th class="text-center">Denní čas</th><th class="text-center">Km</th><th class="text-center">Km/h</th></tr></thead><tbody>';
       $poradi = 1;
	while($dbdata1 = $sth1->fetchObject()){
	    ...
	    $str .= '<td style="padding-left:0" class="text-left"><a href="#" style="text-decoration:none" onclick="detail_cipu_cc('.$dbdata1->cip.','.$this->race_id.','.$this->race_year.','.$_GET['id_etapy'].')">'.$dbdata1->jmeno.'</a></td>';
	    $str .= '<td>'.$dbdata1->lap_time.'</td>';
	    $str .= '<td>'.$dbdata1->race_time.'</td>';
	    $str .= '<td>'.$dbdata1->day_time.'</td>';
	    $str .= '<td>'.($this->delka_kola * $dbdata1->time_order-1).'</td>';
...
function detail_cipu_cc(cip,race_id,race_year,id_etapy) {
    window.open("detail_cipu_cc.php?cip="+cip+"&race_id="+race_id+"&race_year="+race_year+"&id_etapy="+id_etapy);
}
