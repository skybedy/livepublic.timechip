<?php
	...
	if($this->MaxTimeOrder()){
		...
		while($data3 = $sth3->fetchObject()){
		    if($data3->cip > 200 OR $data3->cip > 501){ // v Litovli jsou to čísla pro družstva, které mají v DB vyplněny sloupce "tym"
			$sql4 = "SELECT tymy.nazev_tymu,CONCAT_WS(' ',osoby.prijmeni,osoby.jmeno) AS jmeno,$this->sqlkategorie.nazev_k as nazev_kategorie FROM $this->sqlzavod,tymy,$this->sqlkategorie,osoby WHERE $this->sqlzavod.ido = osoby.ido AND $this->sqlzavod.cip = :cip AND $this->sqlzavod.tym = tymy.id_tymu AND $this->sqlzavod.id_kategorie = $this->sqlkategorie.id_kategorie LIMIT 0,1";
		    }
		    else{ // v Litovli jsou to čísla pro jednotlivce, které nemají v DB vyplněny sloupce "tym", ale "prislusnost" 
			$sql4 = "SELECT tymy.nazev_tymu,CONCAT_WS(' ',osoby.prijmeni,osoby.jmeno) AS jmeno,$this->sqlkategorie.nazev_k as nazev_kategorie FROM $this->sqlzavod,tymy,$this->sqlkategorie,osoby WHERE $this->sqlzavod.ido = osoby.ido AND $this->sqlzavod.cip = :cip AND $this->sqlzavod.prislusnost = tymy.id_tymu AND $this->sqlzavod.id_kategorie = $this->sqlkategorie.id_kategorie LIMIT 0,1";
		    }
		    //echo $sql4."\n\n";
