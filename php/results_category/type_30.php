<?php
	...
	if($category_id == 'all'){
	    ...
	    if($sth->rowCount()){
		$str .= '<table class="table table-bordered  table-hover orlice_cup">';
		$k = 1;
		while($data = $sth->fetchObject()){
		    $sql1 = "SELECT "
			      ...
			      . "$this->sqlzavod.tym = tymy.id_tymu AND " 
                              ...
                              . "$this->sqlvysledky.lap_count <= :time_order " //tohle by mělo zajistit, že se vybere SUM pouze z počtu kol daného požadovaným time_order
			...
			while($dbdata1 = $sth1->fetchObject()){
			    ...
			    $str .= '<td>'.$dbdata1->nazev_tymu.'</td>';
                            $sql2 = "SELECT * FROM $this->sqlvysledky WHERE ids = :ids AND false_time IS NULL AND lap_only IS NULL AND reader = 'CIL' ORDER BY race_time ASC LIMIT 0,$this->time_order";
				...
				if($this->time_order == 1){//pokud je to první čas
				    $str .= '<td><div class="jmeno_ac">'.$zavodnik_pole[$val2->cip]->jmeno.'</div> <div class="cas_ac">'.$val2->lap_time.'</div> <div class="poradi_ac">'.$val2->rank_category_lap.'</div></td>';
