<?php
	...
	if($sth->rowCount()){ //2.if
		...
		if($sth4->rowCount()){ 
			...
			$sql5 = "SELECT CONCAT_WS(' ',osoby.prijmeni,osoby.jmeno) AS jmeno,osoby.psc AS stat,osoby.rocnik,tymy.nazev_tymu,$this->sqlkategorie.nazev_k AS nazev_kategorie,$this->sqlzavod.penalizace,$this->sqlzavod.priznak "
			. "FROM osoby,$this->sqlzavod,tymy,$this->sqlkategorie  WHERE "
			. "$this->sqlzavod.ids = $dbdata4->ids AND "
	 ...
	 $str .= '</tbody></table>';
		...
		if($sth1->rowCount()){
			...
			if($i == 1){
			    ...
			    $str .= '<tr class="header"><th class="text-center">#</th><th class="text-left">Jméno</th>'; //bylo tady i thead, ale v pdf se opakovala hlavička, což bylo na pytel
			    for($k=1;$k<=$max_pocet_kol;$k++){
				$str .= '<th class="text-center">'.$k.'.kolo</th>';
			    }
			    $str .= '</tr><tbody>';
			}
			$sql2 = "SELECT $this->sqlvysledky.lap_time FROM $this->sqlvysledky WHERE cip = :cip AND id_etapy = :id_etapy AND false_time IS NULL  AND time_order > 1 ORDER by time_order ASC";
			$sth2 = $this->db->prepare($sql2);
