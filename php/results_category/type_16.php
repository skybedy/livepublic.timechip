		    ...
		    while($dbdata2 = $sth2->fetchObject()){ //2.cyklus
			    ...
			    */
				...
				while($dbdata3 = $sth3->fetchObject()){
					...
					$str .= '<td rowspan="'.$pocet_clenu.'" class="text-center"><a onclick="detail_ids_cc('.$dbdata2->ids.','.$this->race_id.','.$this->race_year.','.$this->id_etapy.')" href="'.$hash_url.'vysledky">'.$dbdata2->ids.'</a></td>';
			                $str .= '<td><a onclick="detail_cipu_cc('.$dbdata3->cip.','.$this->race_id.','.$this->race_year.','.$this->id_etapy.')" href="'.$hash_url.'vysledky">'.$dbdata3->jmeno.'</a></td>';
