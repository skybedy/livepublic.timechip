<?php
    /*
     * Celkový čas s kolama city sprint, kategorie
     */
	...
	if($sth->rowCount()){
		...
		if($sth1->rowCount()){
		    ...
		    //$str .= '<tr class="header"><th class="text-center">Bib</th><th>Name</th><th class="text-center">Nat</th><th class="text-center">Rnk</th><th class="text-center">Rounds</th><th class="text-center">Time</th><th class="text-center">Gap</th></tr>';
		    $str .= '<tr class="header"><th class="text-center">Bib</th><th>Name</th><th class="text-center">Nat</th><th class="text-center">Rnk</th><th class="text-center">Rounds</th><th class="text-center">Time</th></tr>';
		    $poradi = 1;
		    while($dbdata1 = $sth1->fetchObject()){
                        if($poradi == 1) $best_time = $dbdata1->race_time_sec;
                        $distance_time = $this->DynamicDistances($poradi,$dbdata1->race_time_sec,$best_time);
			...
			if($poradi == 1) $max_pocet_kol = $dbdata1->pocet_kol;// nejvyšší počet kol pro počítání odstupů
				...
				($distance_time != '00:00:00.00') ? ($distance_time = $distance_time) : ($distance_time = '-'); //pokud nemá odstup, tak je první a bude tam pomlčka
			    }
			    else{ // pokud ne, spočítáme odstup v kolech
				
                                
                                $distance_category = $dbdata1->pocet_kol - $max_pocet_kol; 
                                //echo $distance_category;
			    ...
			    $str .= '<td class="text-center">'.substr($dbdata1->finish_time,$this->pocet_ubranych_znaku_zepredu).'</td>';
			   // $str .= '<td class="text-center">'.$distance_time.'</td>';
			    $str .= '</tr>';
			$poradi++;
		    }
		}
	...
	if($sth->rowCount()){
		...
		while($dbdata1 = $sth1->fetchObject()){
			...
			if($sth2->rowCount()){ //pokud je někdo se stejným, tzn. nejvyšším počttem kol, použijeme časový odstup
			    $dbdata2 = $sth2->fetchObject();
			    ($distance_time != '00:00:00.00') ? ($distance_category = $distance_time) : ($distance_category = '-'); //pokud nemá odstup, tak je první a bude tam pomlčka
			}
		    else{ // pokud ne, spočítáme odstup v kolech
			$distance_category = $dbdata1->pocet_kol - $max_pocet_kol; 
			if($distance_category == -1){
