<?php
    /*
     * cross country, normální kola, bez etap
     */
	...
	if($sth->rowCount()){ //2.if
		...
		if($sth2->rowCount()){ //3.if
		    ...
		    $str .= '<tr class="header"><th class="text-center">#</th><th class="text-center">St.č</th><th>Jméno</th><th class="text-center">Tým</th><th class="text-center">Stát</th><th class="text-center">Čas</th><th class="text-center">Kola</th><th class="text-center">Odstup</th><th class="text-center">Km/h</th><th class="text-center">Nejlepší čas</th></tr>';
			...
			$sql3 = "SELECT CONCAT_WS(' ',osoby.prijmeni,osoby.jmeno) AS jmeno,osoby.psc AS stat,tymy.nazev_tymu,$this->sqlkategorie.nazev_k AS nazev_kategorie "
		...
		$str .= '<thead><tr class="header"><th class="text-center">#</th><th class="text-center">St.č</th><th>Jméno</th><th class="text-center">Tým</th><th class="text-center">Stát</th><th class="text-center">Čas</th><th class="text-center">Kolo</th><th class="text-center">Odstup</th><th class="text-center">Km/h</th><th class="text-center">Nejlepší čas</th>';
		$str .= '</tr></thead><tbody>';
		...
		//$str .= $sql2;
		$sth2 =  $this->db->prepare($sql2);
		$sth2->execute(Array(':id_kategorie' => $category_id,':event_order' => $this->event_order));
		if($sth2->rowCount()){ 
		    $poradi = 1;
		    while($dbdata2 = $sth2->fetchObject()){ //2.cyklus
			   $celkova_vzdalenost = $dbdata2->pocet_kol * $this->delka_kola;
		...
		$sth1 =  $this->db->prepare($sql1);
		$sth1->execute(Array(':event_order' => $this->event_order,':id_kategorie' => $category_id));
		if($sth1->rowCount()){
			...
			$sql2 = "SELECT $this->sqlvysledky.lap_time FROM $this->sqlvysledky WHERE cip = :cip ORDER by time_order ASC";
			$sth2 = $this->db->prepare($sql2);
