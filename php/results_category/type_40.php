<?php
    ...
    if($category_id == 'all'){
        $str .= "Jen každá kategorie zvlášť";
    }
	...
	if($sth1->rowCount()){
	    ...
	    $str .= '<h4 class="headline-results">'.$this->race_name.', kategorie '.$dbdata1->nazev_kategorie.'</h4>';
            ...
            //vybereme pocet casu
            $sql2 = "SELECT pocet_casu FROM $this->sqlpodzavody WHERE id_zavodu = :race_id AND poradi_podzavodu = :event_order";
            $sth2 = $this->db->prepare($sql2);
            $sth2->execute(Array(':race_id' => $this->race_id,':event_order' => $this->event_order));
            $dbdata2 = $sth2->fetchObject();
            $time_order = $dbdata2->pocet_casu;
            
            // ted vytahneme pouze tymy prislusne kategorie
            $sql3 = "SELECT tym FROM $this->sqlzavod WHERE poradi_podzavodu = $this->event_order AND id_kategorie = $category_id GROUP BY tym ORDER BY id DESC";
            $sth3 = $this->db->prepare($sql3);
            $sth3->execute(Array(':event_order' => $this->event_order));
            if($sth3->rowCount()){
                while($dbdata3 = $sth3->fetchObject()){
                    $sql4 = "SELECT COUNT(race_time_sec) AS row_count,SUM(race_time_sec) as finish_time FROM (SELECT $this->sqlvysledky.race_time_sec FROM $this->sqlvysledky,$this->sqlzavod WHERE $this->sqlvysledky.cip = $this->sqlzavod.cip AND time_order = :time_order AND $this->sqlzavod.tym = :tym AND $this->sqlzavod.poradi_podzavodu = :event_order AND false_time IS NULL ORDER BY $this->sqlvysledky.race_time_sec LIMIT 0,$this->team_racer_count) AS subquery";
                    $sth4 = $this->db->prepare($sql4);
                    $sth4->execute(Array(':tym' => $dbdata3->tym,':time_order' => $time_order,':event_order' => $this->event_order));
                    if($sth4->rowCount()){
                        while($dbdata2 = $sth4->fetchObject()){
                            if($dbdata2->row_count >= 4){
                                 $tym[$dbdata3->tym] = $dbdata2->finish_time;
                            }
                        }
                    }
                }
            ...
            if($tym){
                ...
                $str .= '<thead><tr class="header"><th class="text-center">#</th><th class="text-left">Název týmu</th><th class="text-left">Složení</th><th class="text-center">Čip</th><th class="text-center">Ročník</th><th class="text-center">Čas</th><th class="text-center">Poř</th><th class="text-center">Celkový čas</th><th class="text-center">Odstup</th></tr></thead><tbody>';
                $poradi = 1;
                                ...
                                while($dbdata4 = $sth4->fetchObject()){
                                    if($k == 1){
                                        ...
                                        $str .= '<td class="text-center">'.$dbdata4->race_time.'</td>';
                                            $str .= '<td class="text-center">'.$dbdata4->rank_category.'</td>';
