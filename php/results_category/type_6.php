<?php
	...
	if($sth->rowCount()){
		...
		$sql1 = "SELECT CONCAT_WS(' ',osoby.prijmeni,osoby.jmeno) AS jmeno,osoby.rocnik,tymy.nazev_tymu,$this->sqlvysledky.ids,$this->sqlvysledky.cip,$this->sqlvysledky.ids_alias,MAX($this->sqlvysledky.race_time) AS finish_time,COUNT($this->sqlvysledky.id) AS pocet_kol,$this->sqlkategorie.nazev_k AS nazev_kategoriez FROM $this->sqlvysledky,$this->sqlzavod,$this->sqlkategorie,tymy,osoby "
			  ...
			  . "AND $this->sqlvysledky.reader LIKE :reader "
			  . "GROUP BY $this->sqlzavod.ids "
			  . "ORDER BY pocet_kol DESC,finish_time ASC".$this->rows_limit;
		 // echo $sql1."\n\n";
		$sth1 =  $this->db->prepare($sql1);
		$sth1->execute(Array(':id_kategorie' => $data->id_kategorie,':event_order' => $this->event_order,':reader' => 'CIL'));
		if($sth1->rowCount()){
		    $class = $k == 1 ? $class = 'nadpis nopadding' : 'nadpis';
		    $str .= '<tr><td class="'.$class.'" colspan="3">'.$data->nazev_kategorie.'</td></tr>';
		    $str .= '<tr class="header"><th class="text-center">#</th><th class="text-center">St.č</th><th>Jméno</th><th>Tým/Bydliště</th><th class="text-center">Ročník</th><th class="text-center">Kola</th><th class="text-center">Km</th><th class="text-center">Poslední start</th><th class="text-center">Celkový čas</th><th class="text-center">Odstup</th></tr>';
		    $poradi = 1;
			...
			if($poradi == 1) $max_pocet_kol = $dbdata1->pocet_kol;// nejvyšší počet kol pro počítání odstupů
			    $sql2 = "SELECT $this->sqlvysledky.distance_category FROM $this->sqlvysledky WHERE $this->sqlvysledky.ids = :ids AND lap_count = :max_pocet_kol"; //vezmeme odstupy, v tomto případě ve zvláštním dotaze
			    $sth2 = $this->db->prepare($sql2);
			    $sth2->execute(Array(':ids' => $dbdata1->ids,':max_pocet_kol' => $max_pocet_kol));
	...
	if($sth->rowCount()){
	    ...
	    $sth1 =  $this->db->prepare($sql1);
	    $sth1->execute(Array(':category_id' => $category_id,':event_order' => $this->event_order,':reader' => 'CIL'));
	    if($sth1->rowCount()){
		$str .= '<tr class="header"><th class="text-center">#</th><th class="text-center">St.č</th><th>Jméno</th><th>Tým/Bydliště</th><th class="text-center">Ročník</th><th class="text-center">Kola</th><th class="text-center">Km</th><th class="text-center">Poslední start</th><th class="text-center">Celkový čas</th><th class="text-center">Odstup</th></tr>';
		...
		while($dbdata1 = $sth1->fetchObject()){
		    ...
		    if($poradi == 1) $max_pocet_kol = $dbdata1->pocet_kol;// nejvyšší počet kol pro počítání odstupů
			$sql2 = "SELECT $this->sqlvysledky.distance_category FROM $this->sqlvysledky WHERE $this->sqlvysledky.ids = '$dbdata1->ids' AND lap_count = '$max_pocet_kol'"; //vezmeme odstupy, v tomto případě ve zvláštním dotaze
			$sth2 = $this->db->prepare($sql2);
			$sth2->execute();
			if($sth2->rowCount()){ //pokud je někdo se stejným, tzn. nejvyšším počttem kol, použijeme časový odstup
			    $dbdata2 = $sth2->fetchObject();
