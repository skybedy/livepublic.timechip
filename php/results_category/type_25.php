<?php
	...
	$sql = "SELECT id_kategorie,kod_k AS kod_kategorie,nazev_k AS nazev_kategorie,poradi_podzavodu FROM $this->sqlkategorie WHERE id_zavodu = :race_id AND poradi_podzavodu = :event_order ORDER BY id_kategorie";
	...
	if($sth->rowCount()){ //2.if
		...
		$sth2->execute(Array(':id_kategorie' => $dbdata1->id_kategorie));
		...
		if($sth2->rowCount()){ //3.if
		    ...
		    while($dbdata2 = $sth2->fetchObject()){ //2.cyklus
			    ...
			    */
			    ...
			    $sth3->execute(Array(':ids' => $dbdata2->ids,':event_order' => $this->event_order));
			    if($sth3->rowCount()){
				...
				while($dbdata3 = $sth3->fetchObject()){
					...
					$str .= '<td rowspan="'.$pocet_clenu.'" class="text-center"><a onclick="detail_ids_cc_bez_etap('.$dbdata2->ids.','.$this->race_id.','.$this->race_year.')" href="'.$hash_url.'vysledky">'.$dbdata2->ids_alias.'</a></td>';
			                $str .= '<td><a onclick="detail_cipu_cc_bez_etap('.$dbdata3->cip.','.$this->race_id.','.$this->race_year.')" href="'.$hash_url.'vysledky">'.$dbdata3->jmeno.'</a></td>';
		    ...
		    while($dbdata2 = $sth2->fetchObject()){ //2.cyklus
			   ...
			   // echo $sql3;
				...
				while($dbdata3 = $sth3->fetchObject()){
					...
					$str .= '<tr>';
					$str .= '<td rowspan="'.$pocet_clenu.'" class="text-center">'.$poradi.'</td>';
					//$str .= '<td rowspan="'.$pocet_clenu.'" class="text-center">'.$dbdata2->ids_alias.'</td>';
					$str .= '<td rowspan="'.$pocet_clenu.'" class="text-center"><a onclick="detail_ids_cc_bez_etap('.$dbdata2->ids.','.$this->race_id.','.$this->race_year.')" href="'.$hash_url.'vysledky">'.$dbdata2->ids_alias.'</a></td>';
