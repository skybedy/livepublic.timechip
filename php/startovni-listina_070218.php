<?php
	...
	public function __construct(){
		...
		$sql1 = "SELECT $this->sqlzavody.kod_zavodu,$this->sqlzavody.delka_kola,$this->sqlzavody.nazev_zavodu,$this->sqlzavody.pocet_podzavodu,$this->sqlzavody.ruzny_pocet_casu FROM $this->sqlzavody WHERE $this->sqlzavody.id_zavodu = :race_id";
		...
		/* provizorka.. mělo by se to dát už do předchozíchi selectu ať se nemusí dělat další dotaz do db*/
		    ...
		    $sql1 = "SELECT $this->sqlzavody.kod_zavodu,$this->sqlzavody.delka_kola,$this->sqlzavody.nazev_zavodu,$this->sqlzavody.pocet_podzavodu,$this->sqlzavody.ruzny_pocet_casu,$this->sqlpodzavody.* FROM $this->sqlzavody,$this->sqlpodzavody WHERE $this->sqlzavody.id_zavodu = :race_id AND $this->sqlpodzavody.id_zavodu = $this->sqlzavody.id_zavodu AND $this->sqlpodzavody.poradi_podzavodu = :event_order";
		    $sth1 =  $this->db->prepare($sql1);
	...
	private function index(){
	    ...
	    $sth1->execute(Array(':race_id' => $this->race_id));
	    if($sth1->rowCount()){
		$fcdata.= '<select id="event_list" class="form-control">';
		$i = 1;
	...
	private function StartingListTypeSelect(){
	    $fcdata = '<select id="starting_list_type" class="form-control">';
	...
	private function StartingListTeribear(){
		...
		if($this->racer_type == 1){
		    ...
		    $sql1 = "SELECT $this->sqlzavod.ids_alias,$this->sqlzavod.cip,CONCAT_WS(' ',osoby.prijmeni,osoby.jmeno) AS jmeno,osoby.rocnik,osoby.psc AS stat,osoby.tricko, $this->sqlkategorie.nazev_k AS nazev_kategorie,tymy.nazev_tymu FROM osoby,$this->sqlzavod,$this->sqlkategorie,tymy WHERE $this->sqlzavod.ido = osoby.ido AND $this->sqlkategorie.poradi_podzavodu = $this->event_order AND $this->sqlkategorie.id_kategorie = $this->sqlzavod.id_kategorie AND $this->sqlzavod.tym_2 = tymy.id_tymu ORDER BY $this->sqlzavod.ids";
		    $sth1 = $this->db->prepare($sql1);
	...
	private function StartingListOverall(){
	   // echo 'hhuh';
		...
		if($this->event_order == 'all'){
		$sql1 = "SELECT $this->sqlzavod.ids_alias,$this->sqlzavod.cip,CONCAT_WS(' ',osoby.prijmeni,osoby.jmeno) AS jmeno,osoby.rocnik,osoby.tricko,osoby.psc AS stat,osoby.tricko,osoby.mail,osoby.telefon,osoby.id_cts,osoby.prijmeni AS surname,osoby.jmeno AS firstname, $this->sqlkategorie.nazev_k AS nazev_kategorie,tymy.nazev_tymu FROM osoby,$this->sqlzavod,$this->sqlkategorie,tymy WHERE $this->sqlzavod.ido = osoby.ido AND $this->sqlkategorie.id_kategorie = $this->sqlzavod.id_kategorie$this->cislo_kategorie AND tymy.id_tymu = $this->sqlzavod.prislusnost ORDER BY $this->sqlzavod.ids";
		//$str .= $sql1;
		$sth1 = $this->db->prepare($sql1);
		$sth1->execute();
		if($sth1->rowCount()){  
		...
		if($this->racer_type == 1){
		   ...
		   //echo $sql1;
			...
			$str .= '<p style="text-align:right">Celkem '.$pocet.' závodníků.</p>';
			$str .= '<table class="table table-striped table-bordered table-hover noborder table_vysledky">';
			$str .= '<thead><tr class="header"><th class="text-center">St.č</th><th class="text-center">Alias</th><th class="text-center">Čip</th><th>Jméno</th><th class="text-center">Ročník</th><th>Tým/Bydliště</th><th class="text-center">Stát</th><th>Kategorie</th></tr></thead>';
			while($dbdata1 = $sth1->fetchObject()){
			    if($this->event_order == 1){
				//$str .= '<tr><td class="text-center">'.$dbdata1->ids.'</td><td class="text-center">'.$dbdata1->ids_alias.'</td><td class="text-center">'.$dbdata1->cip.'</td><td>'.$dbdata1->jmeno.'</td><td class="text-center">'.$dbdata1->rocnik.'</td><td>'.$this->NazevTymu($dbdata1->nazev_tymu).'</td><td class="text-center">'.$dbdata1->stat.'</td><td>'.$dbdata1->nazev_kategorie.'</td></tr>';
		...
		elseif($this->racer_type == 3){  // týmy,použito na Lahofer cup
			...
			$str .= '</tr></thead><tbody>';
			while($data1 = $sth1->fetchObject()){
			    $sql2 = "SELECT CONCAT_WS(' ',osoby.prijmeni,osoby.jmeno) AS jmeno,osoby.rocnik,$this->sqlzavod.cip,$this->sqlzavod.ids_alias FROM osoby,$this->sqlzavod WHERE $this->sqlzavod.tym_2 = '$data1->id_tymu' AND $this->sqlzavod.ido = osoby.ido ORDER BY $this->sqlzavod.cip";
			    $sth2 = $this->db->prepare($sql2);
			    $sth2->execute();
			    if($sth2->rowCount()){
				$pocet_clenu = $sth2->rowCount();
	...
	private function CategoryListSelect(){
	    $fcdata = '';
	    $sql1 = "SELECT id_kategorie,nazev_k FROM $this->sqlkategorie WHERE id_zavodu = :race_id AND poradi_podzavodu = :event_order ORDER BY poradi";
	    $sth1 = $this->db->prepare($sql1);
	    $sth1->execute(Array(':race_id' => $this->race_id,':event_order' => $_GET['event_order']));
	    if($sth1->rowCount()){
		$fcdata.= '<select id="category_list" class="form-control">';
